      ...
      switch (node.kind) {
        case ts.SyntaxKind.ImportEqualsDeclaration:
          ...
          if (importEqualsDeclaration.moduleReference.kind ===
            ...
            symbols.set(importEqualsDeclaration.name.text, <MetadataValue>{__symbolic: 'reference', module: from});
          } else {
            symbols.set(
                importEqualsDeclaration.name.text,
                <MetadataValue>{__symbolic: 'error', message: `Unsupported import syntax`});
          }
          break;
        case ts.SyntaxKind.ImportDeclaration:
          ...
          if (importDecl.importClause.name) {
            ...
            symbols.set(
                ...
                <MetadataValue>{__symbolic: 'reference', module: from, default: true});
          }
            ...
            switch (bindings.kind) {
              case ts.SyntaxKind.NamedImports:
                ...
                for (const binding of (<ts.NamedImports>bindings).elements) {
                  symbols.set(binding.name.text, <MetadataValue>{
              ...
              case ts.SyntaxKind.NamespaceImport:
                ...
                symbols.set(
                    ...
                    <MetadataValue>{__symbolic: 'reference', module: from});
                break;
            }
          }
          break;
...
function populateBuiltins(symbols: Map<string, MetadataValue>) {
  ...
  ['Object', 'Function', 'String', 'Number', 'Array', 'Boolean', 'Map', 'NaN', 'Infinity', 'Math',
   ...
   'Float64Array']
      .forEach(name => symbols.set(name, <MetadataValue>{__symbolic: 'reference', name}));
}
